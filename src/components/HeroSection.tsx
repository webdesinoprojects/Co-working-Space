...
const FALLBACK_IMAGES = {
  left: "https://images.unsplash.com/photo-1497366754035-f200968a6e72?auto=format&fit=crop&w=800&q=80",
  topRight: "https://images.unsplash.com/photo-1524758631624-e2822e304c36?auto=format&fit=crop&w=800&q=80",
  bottomRight: "https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80",
};

export function HeroSection({ data }: { data?: HeroSectionVM }) {
  const badge = data?.badge_text ?? "Axion Workspace";
  const headline = data?.headline ?? "We craft professional environments for teams ready to dominate their category.";
  ...
  const leftImg = data?.left_image?.url ?? FALLBACK_IMAGES.left;
  const leftAlt = data?.left_image?.alt || "Premium Coworking Desk";
  const topRightImg = data?.top_right_image?.url ?? FALLBACK_IMAGES.topRight;
  const topRightAlt = data?.top_right_image?.alt || "Meeting Room Lounge";
  const bottomRightImg = data?.bottom_right_image?.url ?? FALLBACK_IMAGES.bottomRight;
  const bottomRightAlt = data?.bottom_right_image?.alt || "Office Details";
      ...
      <div className="relative z-20 max-w-[1440px] w-full mx-auto px-5 sm:px-8 lg:px-12 pt-[140px] pb-14 sm:pb-16 flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8">
        ...
        <div className="w-full lg:w-[55%] flex flex-col items-start">
          ...
          <div className="mt-8 sm:mt-10 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5">
            <a
              href={ctaHref}
              className="group flex items-center gap-4 bg-[#F26522] hover:bg-[#e05a1a] text-white text-[14px] sm:text-[15px] font-medium rounded-full pl-6 pr-2 py-2.5 transition-colors duration-300 shadow-xl shadow-[#F26522]/20"
            >
              <div className="h-[22px] overflow-hidden flex flex-col">
                <span className="transition-transform duration-500 ease-[cubic-bezier(0.25,0.1,0.25,1)] group-hover:-translate-y-[22px]">{ctaLabel}</span>
                <span className="transition-transform duration-500 ease-[cubic-bezier(0.25,0.1,0.25,1)] group-hover:-translate-y-[22px]">{ctaLabel}</span>
              </div>
            ...
            <div className="flex items-center gap-3 bg-white/90 backdrop-blur-md px-4 py-2.5 rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-white/40 hover:shadow-[0_8px_32px_rgba(0,0,0,0.1)] transition-shadow duration-300">
              ...
              <div className="flex flex-col">
                <span className="text-[14px] sm:text-[15px] font-medium text-gray-900 leading-tight">{promoTitle}</span>
                <span className="text-[11px] sm:text-[12px] bg-gray-900 text-white px-2 py-0.5 rounded w-fit mt-1">{promoBadge}</span>
              </div>
            </div>
          </div>
        </div>
