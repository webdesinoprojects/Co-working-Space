...
import {
  updateAboutStoryTextAction,
  assignAboutStoryImageAction,
} from "@/features/admin/about/actions";
import type { AdminAboutStorySectionVM, MediaAssetVM } from "@/features/admin/types";
...
const STORY_SLOTS = [
  ...
  { slot: 3, label: "Bottom left" },
  { slot: 4, label: "Bottom right" },
] as const;

export function StoryForm({ data }: { data: AdminAboutStorySectionVM }) {
  ...
  const actionHandler = async (formData: FormData) => {
    const result = await updateAboutStoryTextAction(
      Object.fromEntries(formData.entries())
    );
    if (result.success) {
      showToast("Saved", "Story text updated.", "success");
      router.refresh();
    } else {
  ...
  const handleMediaSelect = async (asset: MediaAssetVM) => {
    ...
    const result = await assignAboutStoryImageAction({
      section_id: data.id,
      slot,
      image_asset_id: asset.id,
    });
    ...
    <form action={actionHandler} className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        <div className="grid grid-cols-3 divide-x divide-white/30">
          {/* Text fields */}
          <div className="col-span-2 p-8 pb-12 space-y-5">
            <div className="grid grid-cols-2 gap-5">
              <div>
                <label className={LABEL}>Subheading (orange badge)</label>
                <input
                  name="subheading"
            ...
            <div>
              <label className={LABEL}>Body Paragraph 1</label>
              <textarea
                name="body_1"
                defaultValue={data.body_1}
                required
                rows={3}
            ...
            <div>
              <label className={LABEL}>Body Paragraph 2</label>
              <textarea
                name="body_2"
                defaultValue={data.body_2}
                required
                rows={3}
                className={`${INPUT} resize-y`}
              />
            ...
            <div>
              <label className={LABEL}>Body Paragraph 3</label>
              <textarea
                name="body_3"
                defaultValue={data.body_3}
                required
                rows={3}
                className={`${INPUT} resize-y`}
              />
            ...
            <div className="border-t border-white/30 pt-5">
              <p className={`mb-3 ${LABEL}`}>Belief Cards</p>
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className={LABEL}>Belief 1 Title</label>
                  <input
                    name="belief_1_title"
                    defaultValue={data.belief_1_title}
                    required
                    className={INPUT}
                  />
                </div>
                <div>
                  <label className={LABEL}>Belief 1 Text</label>
                  <textarea
                    name="belief_1_text"
                    defaultValue={data.belief_1_text}
                    required
                    rows={2}
                    className={`${INPUT} resize-y`}
                  />
                ...
                <div>
                  <label className={LABEL}>Belief 2 Title</label>
                  <input
                    name="belief_2_title"
                    defaultValue={data.belief_2_title}
                    required
                    className={INPUT}
                  />
                </div>
                <div>
                  <label className={LABEL}>Belief 2 Text</label>
                  <textarea
                    name="belief_2_text"
                    defaultValue={data.belief_2_text}
                    required
                    rows={2}
                    className={`${INPUT} resize-y`}
                  />
          ...
          <div className="col-span-1 p-6 pb-12 space-y-4">
            <p className={LABEL}>Story Images (2x2 grid)</p>
              ...
              return (
                <div key={slot}>
                  ...
                  <button
                    ...
                    className="w-full aspect-square rounded-xl border-2 border-dashed border-neutral-300 bg-white/40 relative overflow-hidden group hover:border-neutral-500 transition-colors"
                  >
                    {img ? (
                      <>
                        <img
