...
import type { ServiceItemVM, ServicesPageVM } from "@/features/services/types";
...
function toPublicImage(row: ImgRow | null | undefined): PublicImageVM | null {
  if (!row) return null;
  return {
...
function imageFromMap(
  ...
  return toPublicImage(mediaMap[assetId]);
}

export async function getServicesPage(): Promise<ServicesPageVM> {
  const supabase = await createSupabaseServerClient();
  const { data: pageData, error: pageError } = await supabase
    .from("services_pages")
    .select(
      "badge_text, headline, intro_text, primary_cta_label, primary_cta_href, secondary_cta_label, secondary_cta_href, hero_image_asset_id, highlights, services_badge_text, services_title, services_intro_text"
    )
    .eq("page_key", "default")
    .single();
  ...
  const items: ServiceItemVM[] = rows
    ...
    .map((item) => ({
      ...
      image: imageFromMap(mediaMap, item.image_asset_id),
  ...
  return {
    ...
    hero_image: imageFromMap(mediaMap, page.hero_image_asset_id),
    ...
    items: items.length > 0 ? items : SERVICES_PAGE_DEFAULT.items,
  };
}
